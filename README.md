...
The Postman tests are added to the API project as a JSON file. You can find the file at the following location inside the project structure: `src/test/resources/PostmanFramework.json`. You can open Postman and then open a project. Navigate to the folder where you saved the JSON file. It is recommended to save the JSON file from the API project seperately after cloning the repository, so that you can open it in Postman without any errors.
